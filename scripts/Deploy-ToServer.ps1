...
ssh $HostName "powershell.exe -NoProfile -ExecutionPolicy Bypass -File `"$installer`" -InstallRoot `"$InstallRoot`" -BackupRoot `"$BackupRoot`" -GamePort $GamePort -ServerName `"$ServerName`" -WorldName `"$WorldName`""
if ($LASTEXITCODE -ne 0) {
